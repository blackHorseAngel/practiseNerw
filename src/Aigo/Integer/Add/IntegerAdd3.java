...
//       countMap = handleNumsToMap(nums);
    ...
    public  List<List<Integer>>threeSum4(int[] nums){
        ...
        int[]time = array[1];
        //创建结果数组并赋初始值全部为整数最大值，为了避免后面求和为0的问题
//        int[]resultTime = generateResultArray();
        int[]resultTime = new int[num.length];
        //创建结果list
        List<List<Integer>>resultList = new ArrayList<>();
        //轮循数字对象数组，找出三个数字为0的结果并放入结果list中
        calculationNew(num,time,resultList,resultTime);
        return resultList;
    }
    ...
    public List<List<Integer>>threeSumReal(int[]nums){
        int[][]elements = regularize(nums);
        return Collections.emptyList();
    }

    /**
     * 对传入的数组进行整理，将整理的结果保存在一个二维数组中返回 ，每个数组元素包括两个值，一个值是数字本身，另一个是数字出现的次数，对数字从小到大排序
     * @param nums
     * @return
     */
    private int[][] regularize(int[] nums) {
        //nums[i]的范围在-105~105，整体向右平移至没有负数，O（N）复杂度
        int[]elementCounts = new int[211];
        for(int num : nums){
            elementCounts[num+105]++;
        }
        //对统计结果进行处理，生成返回数据
     ...
     * @param resultTime
     */
    private  void calculationNew(int[] num, int[] time, List<List<Integer>> resultList, int[] resultTime) {
        int count = 0;
    ...
    public static void main(String[] args) {
        IntegerAdd3 integerAdd3 = new IntegerAdd3();
        int[]array = {-1,0,1,2,-1,-4};
        List<List<Integer>>list = integerAdd3.threeSum5(array);
